...
Назначение: объяснить, зачем нужно изменение, что должно измениться, что входит и не входит в scope, какие вопросы остаются открытыми. Каждый пункт называет наблюдаемый факт и источник. Дополнительно шаблон требует пользователей, подтверждение, ограничения, зависимости, риски и нецели.
...
Назначение: предложить проверяемые срезы поставки. Каждый пункт называет действие, видимый результат и `AC` или `NFR`. Этот файл помогает подготовить передачу в ASE, но не определяет прохождение gates.
...
The system SHALL на экране дежурного показывать текущее состояние зарегистрированного устройства.

#### Scenario: Устройство недоступно
- **GIVEN** устройство зарегистрировано, дежурный открыл его карточку, последний контакт был успешным
- **WHEN** устройство перестаёт отвечать дольше порога, заданного в требовании
- **THEN** на карточке видно состояние «Недоступно», идентификатор устройства и время последнего контакта
- **AND** дежурный остаётся на той же карточке и может открыть журнал контактов
- **Не происходит:** смена состояния без идентификатора устройства и без времени последнего контакта

#### Scenario: Краткий обрыв связи
- **GIVEN** устройство зарегистрировано и дежурный видит состояние «На связи»
- **WHEN** обрыв короче порога и связь восстанавливается
- **THEN** состояние остаётся «На связи», запись о недоступности не появляется
- **Не происходит:** алерт устойчивой недоступности
```
...
Для подготовки нового входного документа скопируйте форму:

```text
integrations/openspec/fixtures/sample-project/inputs/brd.md
```

Скопируйте её в `inputs/` проекта под именем `REQ-<код>-<краткое-название>.md`, затем заполните подтверждёнными фактами и явно отметьте неизвестные сведения. Пример в конце файла относится к эталонной фикстуре и в рабочий документ не переносится.
