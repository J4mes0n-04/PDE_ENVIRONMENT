...
- Если `config/features.yaml` задаёт `openspec_intake.enabled: true`, до создания Outcome выполните skill `pde-intake-openspec`: change живёт в `workspaces/projects/<project-id>/openspec/changes/<change-id>/`, а Pack появляется только после подтверждения change.
...
- После OpenSpec change запустите `pwsh ./scripts/validate-openspec-change.ps1`.
- После переноса change в Outcome запустите `pwsh ./scripts/map-openspec-to-pack.ps1`.
